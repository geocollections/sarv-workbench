  ...
  methods: {
    ...
    addCopy(object, data = {}, numberOfSave = 0) {
      if (this.validate(object) && !this.sendingData) {
        ...
        this.saveData(object, formData, "add/" + object + "/", true).then(
          savedObjectId => {
            if (savedObjectId) {
              this.addCopyOfRelatedData(
                data,
                savedObjectId,
                object,
                numberOfSave
    ...
    handlePreparationDataBeforeSave(relatedData, data, numberOfSave) {
      let changedRecordIdx = findIndex(data.preparations, function(item) {
        return item.id === relatedData.id;
      });
      relatedData.preparation_number =
        data.preparations[changedRecordIdx].preparation_number +
        "_" +
        numberOfSave;
      return relatedData;
    },
      ...
      Object.entries(data.relatedData).forEach(entry => {
        ...
        if (this.isNotEmpty(this.relatedData[type])) {
          this.relatedData[type].forEach(entry => {
            ...
            if (type === "preparation") {
              savedRelatedData = this.handlePreparationDataBeforeSave(
                savedRelatedData,
                data,
                numberOfSave
              );
            ...
            let copy = this.removeUnnecessaryFields(
              savedRelatedData,
              this.relatedData.copyFields[type]
            );
            ...
            try {
              ...
              console.log(
                "Related data type: " + type + " cannot not be created"
              );
              console.log(e);
            }
          });

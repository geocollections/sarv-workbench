...
export function fetchSpecimens(data, dynamicSearch, timestamp) {
  ...
  if (data.reference && data.reference.trim().length > 0) {
    searchFields += `&multi_search=value:${
      data.reference
    };fields:specimen_reference__reference__reference,specimen_reference__reference__id;lookuptype:${
      data.reference__lookuptype || "icontains"
    }`;
  }
...
export function fetchTaxonCharacters(taxonId, searchParameters) {
  let orderBy = buildOrderBy(
    searchParameters.sortBy,
    searchParameters.sortDesc
  );
  return get(
    // `taxon_character_key/?taxon=${taxonId}&page=${searchParameters.page}&paginate_by=${searchParameters.paginateBy}&order_by=${orderBy}&format=json`
    `taxon_character_key/?taxon=${taxonId}&page=${searchParameters.page}&paginate_by=${searchParameters.paginateBy}&format=json`
  );
}

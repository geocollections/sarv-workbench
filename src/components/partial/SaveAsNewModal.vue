<template>
  <div class="save-as-new-modal">
    <spinner
      v-if="modalSendingData"
      class="loading-overlay"
      size="massive"
      :message="$t('add.overlay')"
    ></spinner>
    ...
    >
      ...
      <div class="row">
        <div class="col">
          ...
          <span class="col-md-4 pr-0 float-right"
            ><b-form-input
              id="rock_en"
      ...
      <div class="row">
        <div class="col">
          ...
          <div class="col-md-4 pr-0 float-right">
            <span v-for="(data, index) in relatedData" :key="index">
              <b-form-checkbox
                v-model="relatedDataChoice[data]"
                :value="true"
                :unchecked-value="false"
                >{{ $t(object + ".relatedTables." + data) }}</b-form-checkbox
              >
            </span>
          </div>
        </div>
      ...
      <div
        class="row"
        v-if="
          relatedDataChoice['preparation'] === true && preparations.length > 0
        "
      >
        <!--BEST would be to implement checking if number is unique-->
        <div class="col-12">
          <label>{{ $t("saveAsNewModal.text3") }}</label>
        </div>
        <div class="col-12">
          <div class="table-responsive-sm">
            <table class="table table-hover table-bordered  related-table">
              ...
              <tbody>
                <tr v-for="(entity, index) in preparations" :key="index">
                  <td>{{ entity.id }}</td>
                  <td>
                    <b-form-input
                      v-model="entity.preparation_number"
                      type="text"
                    />
                  </td>
      ...
      <template slot="modal-footer">
        <b-button variant="outline-danger" @click="hideModal">{{
          this.$t("saveAsNewModal.cancel")
        }}</b-button>
        <b-button
          variant="outline-success"
          @click="
            $root.$emit('save-object-as-new', {
              object: object,
              numberOfCopies: numberOfCopies,
              relatedData: relatedDataChoice,
...
<script>
import { fetchSamplePreparation } from "../../assets/js/api/apiCalls";

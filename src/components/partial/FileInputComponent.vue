  ...
  <div class="row">
    <div class="col-sm-12 mb-2">
            ...
            <li class="mt-2" v-for="(file, key) in files" :key="key">
              <span v-if="file.type.includes('image')">
                <img
                  ...
                  class="img-thumbnail thumbnail-preview"
                  alt="Image preview..."
                />
              </span>
              <span v-else>
      ...
      <b-form-text v-if="!filesState"
        >{{ $t("add.errors.files") }}.</b-form-text
      >
    </div>
  ...
  watch: {
    ...
    fileMetaData: function(newVal) {
      ...
      if (newVal !== null && newVal !== false) {
        // DATE
        if (
          newVal.DateTimeOriginal !== null &&
          typeof newVal.DateTimeOriginal !== "undefined"
        ) {
          let formattedDate = this.$moment(
            newVal.DateTimeOriginal,
            "YYYY:MM:DD HH:mm:ss"
          );
          this.upload.date_created = formattedDate.toDate();
        } else if (
          newVal.DateTime !== null &&
          typeof newVal.DateTime !== "undefined"
        ) {
          let formattedDate = this.$moment(
            newVal.DateTime,
            "YYYY:MM:DD HH:mm:ss"
        ...
        // DESCRIPTION
        if (
          newVal.ImageDescription !== null &&
          typeof newVal.ImageDescription !== "undefined"
        ) {
        ...
        // DEVICE_TXT
        if (newVal.Model !== null && typeof newVal.Model !== "undefined")
          this.upload.device_txt = newVal.Model;
        else if (newVal.Make !== null && typeof newVal.Make !== "undefined")
          this.upload.device_txt = newVal.Make;

        // IMAGE DIMENSIONS
        if (
          newVal.PixelXDimension !== null &&
          typeof newVal.PixelXDimension !== "undefined"
        ) {
        ...
        if (
          newVal.PixelYDimension !== null &&
          typeof newVal.PixelYDimension !== "undefined"
        ) {

  ...
  <div class="header">
    ...
    <b-navbar type="dark" variant="dark" toggleable="lg" fixed="top" style="z-index: 42000">
      ...
      <b-navbar-nav class="always-visible">
        <div v-if="$store.state['shortcuts'].length > 0" v-for="entity in $store.state['shortcuts']">
          <b-nav-item :class="{ 'd-none d-lg-block': !entity.isAlwaysVisible }" :to="{ path: entity.path }" exact>
            {{ entity.title }}
          </b-nav-item>
        </div>
      </b-navbar-nav>
      ...
      <b-navbar-toggle target="nav_dropdown_collapse" class="ml-auto"></b-navbar-toggle>
      ...
      <b-collapse is-nav id="nav_dropdown_collapse" class="text-center">
        ...
        <b-navbar-nav class="ml-auto">
          ...
          <div v-if="$store.state['shortcuts'].length > 0" class="d-lg-none d-xl-none" v-for="entity in $store.state['shortcuts']" >
            <b-nav-item :class="{ 'd-none': entity.isAlwaysVisible }" :to="{ path: entity.path }" exact>
              {{ entity.title }}
            </b-nav-item>
          </div>

...
<!--    <div class="header-sidebar" slot="header"> This makes it fixed  -->
    ...
    <vs-sidebar-group :open="!$route.meta.isEdit" :title="$t('header.title')" id="sidebar-navigation-links">
      ...
      <vs-sidebar-group v-if="isUserAllowedTo('add', 'site')" :title="$t('header.sites')">
        ...
        <vs-sidebar-item v-if="activeSite > 0" index="3.3" icon="fa-map-marker-alt" icon-pack="fas" :to="{ path: '/site/' + activeSite }">
          {{ $t('frontPage.buttons.activeSite') }}: {{ activeSite }}
        </vs-sidebar-item>
      </vs-sidebar-group>

      <!-- ATTACHMENTS -->
      <vs-sidebar-group v-if="isUserAllowedTo('add', 'attachment') || isUserAllowedTo('add', 'imageset')" :title="$t('header.files')">
        <vs-sidebar-item v-if="isUserAllowedTo('add', 'attachment')" index="4.1" icon="fa-folder" icon-pack="far" :to="{ path: '/attachment' }">
          {{ $t('frontPage.buttons.myFiles') }}
        </vs-sidebar-item>
        <vs-sidebar-item v-if="isUserAllowedTo('add', 'attachment')" index="4.2" icon="fa-image" icon-pack="far" :to="{ path: '/attachment/add/photo_archive' }">
          {{ $t('frontPage.buttons.photoArchive') }}
        </vs-sidebar-item>
        <vs-sidebar-item v-if="isUserAllowedTo('add', 'attachment')" index="4.3" icon="fa-image" icon-pack="fas" :to="{ path: '/attachment/add/specimen_image' }">
          {{ $t('frontPage.buttons.specimenImage') }}
        </vs-sidebar-item>
        <vs-sidebar-item v-if="isUserAllowedTo('add', 'attachment')" index="4.4" icon="fa-file" icon-pack="far" :to="{ path: '/attachment/add/other_file' }">
          {{ $t('frontPage.buttons.otherFiles') }}
        </vs-sidebar-item>
        <vs-sidebar-item v-if="isUserAllowedTo('add', 'attachment')" index="4.5" icon="fa-file-pdf" icon-pack="far" :to="{ path: '/attachment/add/digitised_reference' }">
          {{ $t('frontPage.buttons.digitisedReference') }}
        </vs-sidebar-item>

        <vs-sidebar-item v-if="isUserAllowedTo('add', 'imageset')" index="4.6" icon="fa-images" icon-pack="far" :to="{ path: '/imageset/add' }">
          {{ $t('frontPage.buttons.imageset') }}
        </vs-sidebar-item>
      </vs-sidebar-group>

      <!-- REFERENCES -->
      <vs-sidebar-group v-if="isUserAllowedTo('add', 'reference') || isUserAllowedTo('add', 'library') || isUserAllowedTo('add', 'journal')" :title="$t('header.references')">
        <vs-sidebar-item v-if="isUserAllowedTo('add', 'reference')" index="5.1" icon="fa-book" icon-pack="fas" :to="{ path: '/reference' }">
          {{ $t('frontPage.buttons.myReferences') }}
        </vs-sidebar-item>
        <vs-sidebar-item v-if="isUserAllowedTo('add', 'reference')" index="5.2" icon="fa-book-open" icon-pack="fas" :to="{ path: '/reference/add' }">
          {{ $t('frontPage.buttons.reference') }}
        </vs-sidebar-item>

        <vs-sidebar-item v-if="isUserAllowedTo('add', 'library')" index="5.3" icon="fa-list-alt" icon-pack="far" :to="{ path: '/library' }">
          {{ $t('frontPage.buttons.libraries') }}
        </vs-sidebar-item>
        <vs-sidebar-item v-if="isUserAllowedTo('add', 'library')" index="5.4" icon="fa-edit" icon-pack="far" :to="{ path: '/library/add' }">
          {{ $t('frontPage.buttons.library') }}
        </vs-sidebar-item>

        <vs-sidebar-item v-if="isUserAllowedTo('add', 'journal')" index="5.5" icon="fa-newspaper" icon-pack="far" :to="{ path: '/journal/add' }">
      ...
      <vs-sidebar-group v-if="isUserAllowedTo('add', 'locality')" :title="$t('header.localities')">
        <vs-sidebar-item index="8.1" icon="fa-map-marked-alt" icon-pack="fas" :to="{ path: '/locality' }">
          {{ $t('frontPage.buttons.localities') }}
        </vs-sidebar-item>
        <vs-sidebar-item index="8.2" icon="fa-map-marker-alt" icon-pack="fas" :to="{ path: '/locality/add' }">
      ...
      <vs-sidebar-group v-if="isUserAllowedTo('add', 'sample')" :title="$t('header.samples')">
        <vs-sidebar-item index="9.1" icon="fa-vials" icon-pack="fas" :to="{ path: '/sample' }">
          {{ $t('frontPage.buttons.samples') }}
        </vs-sidebar-item>
        <vs-sidebar-item index="9.2" icon="fa-vial" icon-pack="fas" :to="{ path: '/sample/add' }">
          {{ $t('frontPage.buttons.sample') }}
        </vs-sidebar-item>
        <vs-sidebar-item v-if="activeSample > 0" index="9.3" icon="fa-fill-drip" icon-pack="fas" :to="{ path: '/sample/' + activeSample }">
      ...
      <vs-sidebar-group v-if="isUserAllowedTo('add', 'analysis')" :title="$t('header.analyses')">
        <vs-sidebar-item index="10.1" icon="fa-chart-bar" icon-pack="far" :to="{ path: '/analysis' }">
          {{ $t('header.editAnalysis') }}
        </vs-sidebar-item>
        <vs-sidebar-item index="10.2" icon="fa-chart-pie" icon-pack="fas" :to="{ path: '/analysis/add' }">
      ...
      <vs-sidebar-group v-if="isUserAllowedTo('add', 'doi')" :title="$t('header.dois')">
        <vs-sidebar-item index="11.1" icon="fa-database" icon-pack="fas" :to="{ path: '/doi' }">
          {{ $t('header.dois') }}
        </vs-sidebar-item>
        <vs-sidebar-item index="11.2" icon="fa-plus-circle" icon-pack="fas" :to="{ path: '/doi/add' }">
    ...
    <div class="sidebar-scroll-to" v-if="$route.meta.isEdit && activeSearchParams !== null && typeof activeSearchParams.block !== 'undefined' && activeSearchParams.block !== null">
      ...
      <vs-sidebar-item v-for="(value, name, index) in activeSearchParams.block"
                       :key="name"
                       :index="'12.' + index" v-scroll-to="'#block-' + name"
                       :icon="getIcon(name)"

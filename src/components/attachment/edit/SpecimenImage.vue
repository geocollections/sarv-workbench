<template>
  <div class="specimen-image">
    ...
    <div class="row">
      ...
      <div class="col-sm-4 mb-2">
        <vue-multiselect v-model="edit.specimen"
                         id="specimen"
      ...
      <div class="col-sm-4 mb-2">
        <b-form-input id="image_scalebar" v-model="edit.image_scalebar" type="text"></b-form-input>
      </div>
    </div>
    ...
    <div class="row">
      ...
      <!--<div class="col-sm-2">-->
      <!--<label :for="`device`">{{ $t('specimenImage.device') }}:</label>-->
      <!--</div>-->
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      // specimenState() {
      //   return this.edit.specimen !== null
      // },
    ...
    methods: {
      ...
      formatDataForEdit(unformattedData) {
        ...
        if (unformattedData.specimen !== null) uploadableData.specimen = unformattedData.specimen.id
      ...
      getSpecimens(query) {
        if (query.length > 2) {
          ...
          this.$http.get(url).then(response => {
            console.log(response)
            if (response.status === 200) {
              if (response.body.count === 0) this.autocomplete.specimens = []
              else this.autocomplete.specimens = response.body.results
            }
      ...
      buildSpecimen() {
        return {
          id: this.data.specimen__id,
          specimen_id: this.data.specimen__specimen_id,
          coll__number: this.data.specimen__coll__number,
        }
      },

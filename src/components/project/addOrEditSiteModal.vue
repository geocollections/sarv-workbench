      ...
      <div class="col">
        ...
        >
          ...
          {{
            $t(
              $route.meta.isEdit
                ? "edit.buttons.saveAndContinue"
                : "add.buttons.addAnother"
            )
          }}
        </button>
        <button
...
export default {
  ...
  computed: {
    title() {
      if (
        this.$store.state["createRelationWith"].edit === null ||
        typeof this.$store.state["createRelationWith"].edit === "undefined"
      )
        return this.$t("header.sites");
      return (
        this.$t("header.sites") +
        " id:" +
        this.$store.state["createRelationWith"].edit.id
      );
    }
  },
  methods: {

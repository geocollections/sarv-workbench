<template>
  <div
    class="dataset-reference tab-reference row"
    :class="{ active: activeTab === 'dataset_reference' }"
  >
    <div class="col-sm-12" v-if="activeTab === 'dataset_reference'">
      <div class="table-responsive-sm">
        <table class="table table-hover related-table">
          <thead>
            <tr>
              <th>{{ $t("dataset_reference.reference") }}</th>
              <th>{{ $t("dataset_reference.remarks") }}</th>
              <th class="btn-th"></th>
            </tr>
          </thead>
          ...
          <tbody>
            <tr
              v-for="(entity, index) in relatedData.dataset_reference"
              :key="entity.id"
              :class="{
            ...
            <tr class="related-input-data">
              <td>
                <vue-multiselect
                  v-model="relatedData.insert.dataset_reference.reference"
                  id="reference_insert"
                  label="reference"
                  track-by="id"
                  :placeholder="$t('add.inputs.autocomplete')"
              ...
              <td>
                <b-form-input
                  size="sm"
                  v-model="relatedData.insert.dataset_reference.remarks"
                  type="text"
                />
              </td>
